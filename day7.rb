...
addresses.each do |address|
	possible_tls.push address unless address.scan(/((.)((?!\2).)\3\2)/).empty?
end

possible_tls.each do |tl|
	valid_tls.push tl if tl.scan(/\[[^\]]*((.)((?!\2).)\3\2)/).empty?
end
...
p addresses.map{ |addr| checkgroups2(group(addr)) }.count(true)
