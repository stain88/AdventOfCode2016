...
rooms.each do |room|
	matches = room.match(/([a-z-]*)(\d*)\[([a-z]{5})\]/).to_a
	crypt = matches[1].each_char.to_a.sort - ["-"]
	code = matches[2]
	checksum = matches[3]
	lc = Hash.new(0).tap { |c| crypt.each { |letter| c[letter] += 1} }.sort_by { |k,v| [-v,k]}.first(5).to_h.map{ |k,v| "#{k}"}.sort.join("")
	if lc == checksum.split("").sort.join("")
		total += code.to_i
...
real_rooms.each do |room|
	room_name = room.match(/([a-z-]*)/)[1]
	shift_code = room.match(/(?:[a-z-]*)(\d*)/)[1].to_i
	decrypted =  decrypt(room_name, shift_code).join("").strip
	p decrypted, shift_code if decrypted.include? "pole"
end